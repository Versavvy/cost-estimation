"""
build_verification_pdf.py — Render homework-cost-verification.md to a print-ready PDF.

Same rendering path as build_pdf.py (markdown -> styled HTML -> headless Edge/Chrome
--print-to-pdf). No external services.

Usage:
    python build_verification_pdf.py
"""
...
SRC_MD = os.path.join(HERE, "homework-cost-verification.md")
OUT_PDF = os.path.join(HERE, "homework-cost-verification.pdf")
...
p { margin: 6pt 0; }
a { color: #1769aa; text-decoration: none; word-break: break-all; }
strong { color: #0b2e47; }
...
pre { background: #eef3f7; border: 1px solid #d8e2ea; border-radius: 6px; padding: 10pt 12pt;
...
def main() -> int:
    ...
    tmp_html = os.path.join(tempfile.gettempdir(), "homework-cost-verification.html")
